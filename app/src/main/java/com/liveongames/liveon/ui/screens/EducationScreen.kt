...
fun EducationScreen(
    ...
    settingsViewModel: SettingsViewModel = hiltViewModel(), // kept for API compatibility
    onEducationCompleted: () -> Unit = {},
    onDismiss: () -> Unit = {}
) {
    // Alias to keep existing body changes minimal
    val vm = viewModel

    val catalog by vm.catalog.collectAsState()
    val actions by vm.actions.collectAsState()
    val educations by vm.educations.collectAsState()
    val active by vm.activeEducation.collectAsState()
    val overallGpa by vm.overallGpa.collectAsState()
    val term by vm.termState.collectAsState()
    ...
    Column(
        ...
        active?.let { a ->
            val course = catalog.firstOrNull { it.id == a.id } ?: return@let
    ...
    selectedCourse?.let { course ->
        CourseDetailsPanel(
            ...
            onDismiss = {
                selectedCourse = null
                // If you want screen-level dismiss to bubble up, call:
                // onDismiss()
            }
        )
    }
